::$haZZam! A native Windows image deployment tool.
::Build, administrate and backup your Windows in a native WinPE recovery environment. (C) Joshua Cline - All rights reserved
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&SET $VER_CUR=1105
...
CLS&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&CALL:PAD_LINE
IF NOT "%PROG_MODE%"=="RAMDISK" ECHO                     $haZZam^^! Image-Deploy Portable&&CALL:PAD_LINE
...
EXIT /B
:FOLDER_MODE
ECHO.The folder structure will be regenerated&&ECHO.If a file is open or mounted and cannot be moved, it's possible you could lose that data&&ECHO.CLI folder check (CACHE/PACK/IMAGE) will be effected&&ECHO.ARE YOU SURE?&&ECHO.PRESS{Z}TO PROCEED&&CALL:MENU_SELECT
...
IF "%TITLE%"=="7" TITLE  Index Slot-Mode VHDX's named between 0.VHDX and 9.VHDX are detected at boot.
IF "%TITLE%"=="4" TITLE  When locking the VHDX-Host-Partition, windows gets quirky with the pagefile. 
IF "%TITLE%"=="5" TITLE  $haZZam^^! The Mighty Batch
IF "%TITLE%"=="6" TITLE  $haZZam^^! A native Windows image deployment tool.
IF "%TITLE%"=="7" TITLE  In Slot-Mode VHDX's named between 0.VHDX and 9.VHDX are detected at boot.
IF "%TITLE%"=="8" TITLE  Only download from github.com/joshuacline
IF "%TITLE%"=="9" TITLE  $haZZam^^! The Mighty Batch
...
REM $ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK
CLS&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF "%PROG_OPER%"=="COLOR_SLOT" SET /A COLOR_SLOT+=1
IF "%PROG_OPER%"=="COLOR_SLOT" COLOR %COLOR_SLOT%&&IF "%COLOR_SLOT%"=="8" SET "COLOR_SLOT="
...
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&SET "LIVE_APPLY="&&SET "LOL_PARSE="&&SET "LIST_CREATE="&&SET "FILE_HEADER="&&SET "HEADER_TGT="&&SET "HEADER_TGT_LOL="&&SET "ERR_MSG="&&SET "TARGET_IMAGE="
CLS&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF NOT DEFINED PROG_OPER ECHO  [LIST]     {C}reate   {V}iew    {G}o^^!         {X}ISO-Mount&&CALL:PAD_LINE
IF NOT DEFINED PROG_OPER ECHO  [VHDX]{*}%VHDX_BACKUP%]  {I}nspect   {D}ISM   {M}ount/Unmount   {N}ew(Empty)&&CALL:PAD_LINE
...
IF "%SELECT%"=="Z" SET "PROG_OPER=ISOUNMOUNT"
...
IF "%SELECT%"=="*" CALL:IMAGEMGR_VHDX_BACKUP
...
IF "%PROG_OPER%"=="ISOUNMOUNT" (ECHO.select VOLUME %ISO_LETTER%&&ECHO.Remove letter=%ISO_LETTER% noerr&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART"&&SET "PROG_OPER="&&ECHO Drive letter %ISO_LETTER% removed&&CALL:PAUSED&&GOTO:IMAGEMGR_START&&ECHO 
...
IF "%PROG_OPER%"=="VHDXMOUNT" IF EXIST "V:\" CALL:VDISK_DETACH&&SET "PROG_OPER="&&GOTO:IMAGEMGR_START
IF "%PROG_OPER%"=="VHDXMOUNT" SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
...
IF "%PROG_OPER%"=="NEW" IF DEFINED NEW_VDISK IF DEFINED VHDX_SIZE CALL:VDISK_CREATE
...
EXIT /B
:LIST_LOL_CREATE
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.LST"&&ECHO   AVAILABLE LST'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO           Enter File # To Build List, Multiples OK(1 2 3)&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&ECHO.&&CALL:MENU_SELECT
...
EXIT /B
:IMAGEMGR_LIST_MAIN
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! List Creator&&CALL:PAD_LINE&&SET "CORRUPT_FLAG="&&SET "LIST_FMT=NUMERAL"&&SET "LIST_INSERTA= [ * ]\[List-of-Lists]"&&SET "LIST_INSERTB= [ I ]\[ImageApply[$PK]"&&SET "LIST_INSERTC= [ C ]\[ImageApply[CAB]"&&SET "LIST_DIR=%CACHE_FOLDER%\*.MLB"&&SET "LIST_INSERTD= [ M ]\[ImageApply[MSU]"&&SET "LIST_INSERTE= [ S ]\[SetupComplete[$PK]"&&SET "LIST_INSERTF= [ R ]\[RunOnce[$PK]"&&SET "LIST_PAD1=1"&&SET "LIST_INSERTJ= [ # ]\[Create List-Base[MLB]"&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                    Select a {#} To Start a New List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
...
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! List Creator&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%MLB%\*.MST"&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                    Select a {#} To Start a New List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
...
ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                 Enter # To Use As Source For List-Base&&CALL:PAD_LINE&&ECHO               Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
...
IF EXIST "V:\" SET "ERR_MSG=Drive letter V:\ can NOT be in use. Reassign/Unmount the letter in use."&&GOTO:LIST_BASE_CLEANUP
SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
IF NOT EXIST "V:\" SET "ERR_MSG=Virtual Disk Error. If VHDX refuses mounting, reboot and try again."&&CALL:VDISK_DETACH&&GOTO:LIST_BASE_CLEANUP
:LIVE_APPLY_BASE_SKIP
CLS&&SET "MLB=%CACHE_FOLDER%\MLB"&&CALL:PAD_LINE&&ECHO                        Extracting .MLB List-Base....&&CALL:PAD_LINE&&CALL:TITLECARD
IF EXIST "%MLB%" RD /Q /S "\\?\%MLB%">NUL 2>&1
MD "%MLB%">NUL 2>&1
...
DISM /%APPLY_TARGET% /GET-Provisionedappxpackages>"$TMP.TXT"
FOR /F "TOKENS=1-9 DELIMS=: " %%a in ($TMP.TXT) DO (IF "%%a"=="PackageName" CALL ECHO {%%b%%c%%d%%e%%f%%g%%h%%i}>>"%MLB%\AppX.MST")
ECHO   [DONE]&&ECHO   Getting feature listing...
DISM /%APPLY_TARGET% /GET-FEATURES>"$TMP.TXT"
FOR /F "TOKENS=1-9 DELIMS=: " %%a in ($TMP.TXT) DO (IF "%%a %%b"=="Feature Name" CALL ECHO {%%c%%d%%e%%f%%g%%h%%i}>>"%MLB%\Features.MST")
...
CALL:IF_LIVE2
DISM /%APPLY_TARGET% /GET-PACKAGES>"$TMP.TXT"
FOR /F "TOKENS=1-9 DELIMS= " %%a in ($TMP2.TXT) DO (IF "%%b"=="Based" REG ADD "%%a %%b %%c" /V "Visibility" /T REG_DWORD /D "2" /F>NUL 2>&1)
FOR /F "TOKENS=1-9 DELIMS=: " %%a in ($TMP.TXT) DO (IF "%%a %%b"=="Package Identity" CALL ECHO {%%c%%d%%e%%f%%g%%h%%i}>>"%MLB%\Components.MST")
ECHO   [DONE]&&ECHO   Getting service listing...&&CALL:IF_LIVE1
REG QUERY "%HIVE_SYSTEM%\ControlSet001\Services" /f Type /c /e /s>"$TMP.TXT"
FOR /F "TOKENS=1-9 DELIMS=\ " %%a in ($TMP.TXT) DO (IF "%%a"=="HKEY_LOCAL_MACHINE" IF NOT "%%e"=="" CALL SET "OBJECT_NAME=%%e%%f%%g%%h%%i"
IF "%%a"=="Type" IF "%%c"=="0x10" CALL:OBJECT_PARSE
IF "%%a"=="Type" IF "%%c"=="0x20" CALL:OBJECT_PARSE)
ECHO   [DONE]&&ECHO   Getting task listing...
REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tasks" /f Path /c /e /s>"$TMP.TXT"
FOR /F "TOKENS=1* DELIMS=\" %%a in ($TMP.TXT) DO (IF "%%a"=="    Path    REG_SZ    " CALL ECHO {%%b}>>"%MLB%\Tasks.MST")
...
EXIT /B
:OBJECT_PARSE
CALL ECHO {%OBJECT_NAME%}>>"%MLB%\Services.MST"
EXIT /B
...
IF "%BASE_TYPE%"=="SERVICES" ECHO                    Which type of list?&&CALL:PAD_LINE&&ECHO {1}Service-Start Auto&&ECHO {2}Service-Start Manual&&ECHO {3}Disable Service&&ECHO {4}Delete Service&&ECHO {5}Start Service(LIVE-TEST)&&ECHO {6}Stop Service(LIVE-TEST)&&SET "PROMPT_SET=BAT_SELECT"&&CALL:PROMPT_SET
...
COPY /Y "%MLB%\%MAST_NAME%" "$TMP.TXT">NUL 2>&1
IF "%BASE_TYPE%"=="APPX" SET "PARSE_CNT="&&FOR /F "TOKENS=1-9 SKIP=1 DELIMS={}" %%a in ($TMP.TXT) DO (CALL SET "%BASE_TYPE%_NAME=%%a"&&CALL:PARSE_QUERY)
IF "%BASE_TYPE%"=="FEATURES" SET "PARSE_CNT="&&FOR /F "TOKENS=1-9 SKIP=1 DELIMS={}" %%a in ($TMP.TXT) DO (CALL SET "%BASE_TYPE%_NAME=%%a"&&CALL:PARSE_QUERY)
IF "%BASE_TYPE%"=="PACKAGES" SET "PARSE_CNT="&&FOR /F "TOKENS=1-9 SKIP=1 DELIMS={}" %%a in ($TMP.TXT) DO (CALL SET "%BASE_TYPE%_NAME=%%a"&&CALL:PARSE_QUERY)
IF "%BASE_TYPE%"=="SERVICES" SET "PARSE_CNT="&&FOR /F "TOKENS=1-9 SKIP=1 DELIMS={}" %%a in ($TMP.TXT) DO (CALL SET "%BASE_TYPE%_NAME=%%a"&&CALL:PARSE_QUERY)
IF "%BASE_TYPE%"=="TASKS" SET "PARSE_CNT="&&FOR /F "TOKENS=1-9 SKIP=1 DELIMS={}" %%a in ($TMP.TXT) DO (CALL SET "%BASE_TYPE%_NAME=%%a"&&CALL:PARSE_QUERY)
DEL /F "$TMP.TXT">NUL 2>&1
...
CALL:PAD_LINE&&ECHO                     Enter File # To Select A List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" EXIT /B
...
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! File Picker&&CALL:PAD_LINE&&CALL:HEADER_PARSE>NUL 2>&1
SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&SET "LIST_INSERTA= [ @ ]\[Current-Environment]"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                      Enter File # To Select Target&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&ECHO.&&CALL:MENU_SELECT
CLS&&CALL:PAD_LINE&&ECHO                            Image-Management&&CALL:PAD_LINE
IF "%SELECT%"=="@" SET "LIVE_APPLY=1"&&GOTO:THE_ACTION
IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF EXIST "V:\" SET "ERR_MSG=Drive letter V:\ can NOT be in use. Reassign/Unmount the letter in use."&&GOTO:THE_ACTION_CLEANUP
...
SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
IF NOT EXIST "V:\" SET "ERR_MSG=Virtual Disk Error. If VHDX refuses mounting, reboot and try again."&&CALL:VDISK_DETACH&&GOTO:THE_ACTION_CLEANUP
:THE_ACTION
IF DEFINED LOL_PARSE CALL:LOL_RUNNER
IF NOT DEFINED LOL_PARSE CALL:IMAGEMGR_LIST_OPER
CALL:MOUNT_INT
...
ECHO  {1}RestoreHealth&&ECHO  {2}Cleanup&&ECHO  {3}ResetBase&&SET "PROMPT_SET=DISM_MENU"&&CALL:PAD_LINE&&CALL:PROMPT_SET
...
SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&SET "LIST_INSERTA= [ @ ]\[Current-Environment]"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                           Enter File # to Pick&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&ECHO.&&CALL:MENU_SELECT
IF "%SELECT%"=="@" SET "LIVE_APPLY=1"&&SET "APPLY_TARGET=ONLINE"&&GOTO:IMAGEMGR_DISM_OPER_JUMP
IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" EXIT /B
IF EXIST "V:\" SET "ERR_MSG=Drive letter V:\ can NOT be in use. Reassign/Unmount the letter in use."&&GOTO:DISM_OPER_CLEANUP
SET "APPLY_TARGET=IMAGE:V:"&&SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
IF NOT EXIST "V:\" SET "ERR_MSG=Virtual Disk Error. If VHDX refuses mounting, reboot and try again."&&CALL:VDISK_DETACH&&GOTO:DISM_OPER_CLEANUP
...
EXIT /B
:IMAGEMGR_INSPECT
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&SET "LIST_INSERTA= [ @ ]\[Current-Environment]"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                        Enter File # To Inspect&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&ECHO.&&CALL:MENU_SELECT&&CLS
IF "%SELECT%"=="@" SET "LIVE_APPLY=1"&&SET "TARGET_IMAGE=LIVE"&&GOTO:INSPECT_JUMP_LIVE
IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" EXIT /B
IF EXIST "V:\" SET "ERR_MSG=Drive letter V:\ can NOT be in use. Reassign/Unmount the letter in use."&&CALL:PAD_LINE&&ECHO.&&GOTO:INSPECT_CLEANUP
...
IF NOT EXIST "V:\" SET "ERR_MSG=Virtual Disk Error. If VHDX refuses mounting, reboot and try again."&&CALL:VDISK_DETACH&&GOTO:INSPECT_CLEANUP
:INSPECT_JUMP_LIVE
...
CALL:PAD_LINE&&ECHO                         Getting Appx Info...&&CALL:PAD_LINE&&CALL:TITLECARD&&CALL:TEXTCOLOR
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
...
IF EXIST "V:\" SET "ERR_MSG=Drive letter V:\ can NOT be in use. Reassign/Unmount the letter in use."&&GOTO:IMAGEPROC_CLEANUP
...
CLS&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
CALL:PAD_LINE&&ECHO                         $haZZam^^! Boot-Creator&&CALL:PAD_LINE
...
ECHO.&&ECHO Saving boot-media...&&CALL:TITLECARD_NML
...
:BOOT_CLEANUP
ECHO.&&ECHO Unmounting EFI...&&CALL:TITLECARD_NML&&CALL:VDISK_DETACH>NUL 2>&1
...
REM FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_
REM FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_
@ECHO OFF&&CALL:COLOR_CHK&&CLS&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF "%PROG_OPER%"=="FILEMGR_COPY" IF NOT EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Copying [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_TARGET%]...&&XCOPY "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_TARGET%\" /C /Y
IF "%PROG_OPER%"=="FILEMGR_COPY" IF EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Copying [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_TARGET%]...&&XCOPY "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_TARGET%\%SELECT%\" /E /C /I /Y
IF "%PROG_OPER%"=="FILEMGR_DUPE" IF NOT EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Copying [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_SOURCE%\Copy_%SELECT%]...&&XCOPY "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_SOURCE%\Copy_%SELECT%\" /C /Y
IF "%PROG_OPER%"=="FILEMGR_DUPE" IF EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Copying [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_SOURCE%\Copy_%SELECT%]...&&XCOPY "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_SOURCE%\Copy_%SELECT%\" /E /C /I /Y
IF "%PROG_OPER%"=="FILEMGR_MOVE" IF NOT EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Moving [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_TARGET%]...&&MOVE /Y "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_TARGET%\"
IF "%PROG_OPER%"=="FILEMGR_MOVE" IF EXIST "%FILEMGR_SOURCE%\%SELECT%\*" ECHO Moving [%FILEMGR_SOURCE%\%SELECT%]to[%FILEMGR_TARGET%]...&&XCOPY "%FILEMGR_SOURCE%\%SELECT%" "%FILEMGR_TARGET%\%SELECT%\" /E /C /I /Y
...
EXIT /B
:PACKEX_MENU_START
@ECHO OFF&&CALL:COLOR_CHK&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool&&CALL:PACKEX_MENU_DISPLAY
