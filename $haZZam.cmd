...
EXIT /B
:IMAGEMGR_LIST_MAIN
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! List Creator&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_INSERTA= [ # ]\[List-of-Lists]"&&SET "LIST_INSERTB= [ I ]\[ImageApply $PK]"&&SET "LIST_INSERTC= [ C ]\[ImageApply CAB]"&&SET "LIST_DIR=%CACHE_FOLDER%\*.MST"&&SET "LIST_INSERTD= [ M ]\[ImageApply MSU]"&&SET "LIST_INSERTE= [ S ]\[SetupComplete $PK]"&&SET "LIST_INSERTF= [ R ]\[RunOnce $PK]"&&SET "LIST_PAD1=1"&&SET "LIST_INSERTJ= [ * ]\[List-Base MST]"&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                    Select a {#} To Start a New List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
...
CLS&&CALL:PAD_LINE&&ECHO                        Extracting List-Base....&&CALL:PAD_LINE&&CALL:TITLECARD
...
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
FOR /F "TOKENS=1-9 DELIMS= " %%a in ($TMP.TXT) DO (IF "%%b"=="Based" REG ADD "%%a %%b %%c" /V "Visibility" /T REG_DWORD /D "2" /F>NUL 2>&1)
...
CALL:MOUNT_INT&&CALL:VDISK_DETACH&&CALL:PAD_LINE&&ECHO          The Following List-Base Extracts(.MST) Were Created:&&CALL:PAD_LINE&&CALL:TITLECARD
