...
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&CHCP 437>NUL&&SET $VER_CUR=1117&&SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&Reg.exe query "HKU\S-1-5-19\Environment">NUL
...
IF DEFINED LIST_DIR IF EXIST "%LIST_DIR%" SET "LIST_CNT=0"&&DIR "%LIST_DIR%" /A: /B /O:GN>%TEMP%\$HZ&&FOR /F "TOKENS=*" %%a in (%TEMP%\$HZ) DO (CALL SET /A LIST_CNT+=1&&CALL SET "OBJ_NAME=%%a"&&CALL:FILE_LIST_OUTSIDE)
IF EXIST "%TEMP%\$HZ" DEL /Q /F "%TEMP%\$HZ">NUL 2>&1
...
EXIT /B
:TEST_SELECT
ECHO TEST&&ECHO (0) TO EXIT&&SET COMMAND99=&&SET /P "COMMAND99=$>>"
...
:$ETTINGS_START
REM $ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS
CLS&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&CALL:PAD_LINE&&ECHO                         Settings Configuration&&CALL:PAD_LINE
FOR %%a in (AUTOPILOT BRUTE_FORCE MACRO_TOGGLE) DO (IF NOT DEFINED %%a SET "%%a=DISABLED")
...
IF NOT DEFINED PROG_OPER ECHO  {F}older Layout  [%FOLDER_MODE%]&&ECHO  {B}rute TSK/SVC  [%BRUTE_FORCE%]
...
IF "%PROG_OPER%"=="AUTOPILOT_ENABLE" IF NOT EXIST "%PROG_SOURCE%\AutoPilot.cmd" ECHO No AutoPilot.cmd exists in the program folder. From the menu, press {V} to generate&&SET "PROG_OPER="&&CALL:PAUSED&&GOTO:$ETTINGS_START
...
:IMAGEMGR_START
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
@ECHO OFF&&CLS&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&CALL:TITLEGEN&&SET "PROBLEM_SVC=EDGEUPDATE EDGEUPDATEM WDNISSVC WINDEFEND WMPNETWORKSVC"&&SET "LIVE_APPLY="&&SET "SANDWICH_PARSE="&&SET "LOL_PARSE="&&SET "LIST_CREATE="&&SET "FILE_HEADER="&&SET "HEADER_TGT="&&SET "HEADER_TGT_LOL="&&SET "ERR_MSG="&&SET "TARGET_IMAGE="&&CALL:PAD_LINE&&ECHO                            Image Management&&CALL:PAD_LINE&&IF NOT DEFINED VHDX_BACKUP SET VHDX_BACKUP=ORIG
...
IF DEFINED PROG_OPER ECHO                    Enter File # To [%PROG_OPER%] {/}Type&&CALL:PAD_LINE&&CALL:TITLECARD
CALL:PAD_PREV&&CALL:MENU_SELECT&&IF DEFINED PROG_OPER CALL:FILE_LIST>NUL
IF NOT DEFINED PROG_OPER IF NOT DEFINED SELECT GOTO:PROG_MAIN
IF DEFINED PROG_OPER IF NOT DEFINED SELECT SET "PROG_OPER="
...
IF "%SELECT%"=="A" IF "%VHDX_BACKUP%"=="COPY" SET "VHDX_BACKUP=ORIG"&&GOTO:IMAGEMGR_START
IF "%SELECT%"=="A" IF "%VHDX_BACKUP%"=="ORIG" SET "VHDX_BACKUP=COPY"&&GOTO:IMAGEMGR_START
...
GOTO:IMAGEMGR_START
:LIST_STACKER
CLS&&CALL:PAD_LINE&&ECHO                               File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.LST"&&ECHO   AVAILABLE LST'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO           Enter File # To Build List, Multiples OK(1 2 3)&&CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:MENU_SELECT
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
...
IF "%LIST_CREATE%"=="LOL" SET "PARSE_CNT=0"&&FOR %%a in (%SELECT%) DO (CALL SET TEMPTARGET=%%TEMP_NAME%%a%%&&CALL:LIST_LOL)
IF "%LIST_CREATE%"=="SANDWICH" SET "PARSE_CNT=0"&&FOR %%a in (%SELECT%) DO (CALL SET TEMPTARGET=%%TEMP_NAME%%a%%&&CALL:THE_DELI_1)
...
IF NOT EXIST "%CACHE_FOLDER%\%TEMPTARGET%" EXIT /B
SET "HEADER_TGT=%CACHE_FOLDER%\%TEMPTARGET%"&&CALL:HEADER_PARSE
SET "SANDWICH_LAYER="&&SET "COMMENT=- [LAYER CHANGE TRIGGER]"&&FOR /F "TOKENS=*" %%a in (%HEADER_TGT%) DO (CALL SET "SANDWICH_MEAT="&&IF NOT "%%a"=="" IF NOT "%%a"==" " IF NOT "%%a"=="  " CALL SET "SANDWICH_MEAT=%%a"&&CALL:THE_DELI_2)
EXIT /B
:THE_DELI_2
...
EXIT /B
:IMAGEMGR_LIST_MAIN
CLS&&CALL:PAD_LINE&&ECHO                              List Creator&&CALL:PAD_LINE&&ECHO.&&ECHO  [ C ]\[ImageApply[CAB]&&ECHO  [ M ]\[ImageApply[MSU]&&ECHO  [ I ]\[ImageApply[$PK]&&ECHO  [ S ]\[SetupComplete[$PK]&&ECHO  [ R ]\[RunOnce[$PK]&&ECHO.&&CALL:PAD_LINE&&ECHO  [ - ]\[List-of-Lists]&&ECHO  [ + ]\[Sandwich-List]&&ECHO  [ # ]\[Difference-List]&&CALL:PAD_LINE&&SET "LIST_INSERTA= [ * ]\[Create List-Base[MLB]"&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.MLB"&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                    Select a {#} To Start a New List&&CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:MENU_SELECT
IF "%SELECT%"=="*" SET "LIST_CREATE=LIST-BASE"&&CALL:LIST_BASE_CREATE
IF "%SELECT%"=="-" SET "LIST_CREATE=LOL"&&CALL:LIST_STACKER
IF "%SELECT%"=="+" SET "LIST_CREATE=SANDWICH"&&CALL:LIST_STACKER
IF "%SELECT%"=="#" SET "LIST_CREATE=LIST-DIFF"&&SET "LIST_PASS=1"&&CALL:LIST_DIFFERENCER
...
MD "%MLB%">NUL 2>&1
SET "CORRUPT_FLAG="&&DISM /APPLY-IMAGE /IMAGEFILE:"%MLB_NAME%" /INDEX:1 /APPLYDIR:"%MLB%">NUL 2>&1
IF NOT EXIST "%MLB%\*.MST" ECHO [%SELECT%] is corrupt&&SET "CORRUPT_FLAG=1"&&RD /Q /S "\\?\%MLB%">NUL 2>&1
IF DEFINED CORRUPT_FLAG CALL:PAUSED&EXIT /B
CLS&&CALL:PAD_LINE&&ECHO                              List Creator&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%MLB%\*.MST"&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                    Select a {#} To Start a New List&&CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:MENU_SELECT
...
EXIT /B
:LIST_DIFFERENCER
CLS&&CALL:PAD_LINE&&ECHO                           List-base Difference&&CALL:PAD_LINE&&ECHO.&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.MLB"&&CALL:FILE_LIST&&CALL:PAD_LINE
IF "%LIST_PASS%"=="1" SET "MLB2="&&ECHO                      Select a MLB {#} to reference
IF "%LIST_PASS%"=="2" ECHO             Select a MLB {#} to difference (reverse-clone)
CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:MENU_SELECT
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" EXIT /B
IF EXIST "%CACHE_FOLDER%\%SELECT%" SET "MLB%LIST_PASS%=%CACHE_FOLDER%\%SELECT%"&&MD "%CACHE_FOLDER%\MLB">NUL
SET "CORRUPT_FLAG="&&DISM /APPLY-IMAGE /IMAGEFILE:"%CACHE_FOLDER%\%SELECT%" /INDEX:1 /APPLYDIR:"%CACHE_FOLDER%\MLB">NUL 2>&1
IF NOT EXIST "%CACHE_FOLDER%\MLB\*.MST" ECHO [%SELECT%] is corrupt&&SET "CORRUPT_FLAG=1"
RD /Q /S "\\?\%CACHE_FOLDER%\MLB">NUL 2>&1
IF DEFINED MLB2 IF "%MLB1%"=="%MLB2%" ECHO [%MLB1%] and [%MLB2%] are the same...&&SET "CORRUPT_FLAG=1"
IF DEFINED CORRUPT_FLAG CALL:PAUSED&EXIT /B
IF "%LIST_PASS%"=="1" SET /A LIST_PASS+=1&&GOTO:LIST_DIFFERENCER
CALL:PAD_LINE&&ECHO                            Name of the list?&&CALL:PAD_LINE&&ECHO.&&SET "PROMPT_SET=NEW_NAME"&&CALL:PROMPT_SET_ANY
IF NOT DEFINED NEW_NAME EXIT /B
ECHO Differencing [%MLB1%]/[%MLB2%]...&&MD "%CACHE_FOLDER%\MLB1">NUL&&MD "%CACHE_FOLDER%\MLB2">NUL
DISM /APPLY-IMAGE /IMAGEFILE:"%MLB1%" /INDEX:1 /APPLYDIR:"%CACHE_FOLDER%\MLB1">NUL 2>&1
DISM /APPLY-IMAGE /IMAGEFILE:"%MLB2%" /INDEX:1 /APPLYDIR:"%CACHE_FOLDER%\MLB2">NUL 2>&1
DIR "%CACHE_FOLDER%\MLB1\*.MST" /A: /B /O:GN>$HZ&&FOR /F "TOKENS=*" %%a in ($HZ) DO (CALL SET "SRC_X=%%a"&&CALL:LIST_DIFF2)
RD /Q /S "%CACHE_FOLDER%\MLB1">NUL&&RD /Q /S "%CACHE_FOLDER%\MLB2">NUL&&DEL /F $HZ>NUL
EXIT /B
:LIST_DIFF2
SET "TGT_LST="&&CALL SET /P HEADER=<"%CACHE_FOLDER%\MLB1\%SRC_X%"
IF "%HEADER%"=="APPX-MASTER" SET "TGT_LST=APPX"&&ECHO.APPX-DELETE>"%CACHE_FOLDER%\%NEW_NAME%_APPX.LST"
IF "%HEADER%"=="FEATURES-MASTER" SET "TGT_LST=FEATURES"&&ECHO.FEATURES-DISABLE>"%CACHE_FOLDER%\%NEW_NAME%_FEATURES.LST"
IF "%HEADER%"=="PACKAGES-MASTER" SET "TGT_LST=PACKAGES"&&ECHO.PACKAGES-DELETE>"%CACHE_FOLDER%\%NEW_NAME%_PACKAGES.LST"
IF "%HEADER%"=="SERVICES-MASTER" SET "TGT_LST=SERVICES"&&ECHO.SERVICES-DELETE>"%CACHE_FOLDER%\%NEW_NAME%_SERVICES.LST"
IF "%HEADER%"=="TASKS-MASTER" SET "TGT_LST=TASKS"&&ECHO.TASKS-DELETE>"%CACHE_FOLDER%\%NEW_NAME%_TASKS.LST"
COPY /Y "%CACHE_FOLDER%\MLB1\%SRC_X%" $TMP1>NUL&&COPY /Y "%CACHE_FOLDER%\MLB2\%SRC_X%" $TMP2>NUL
FOR /F "TOKENS=* SKIP=1" %%a in ($TMP1) DO (SET "MCH="&&FOR /F "TOKENS=* SKIP=1" %%1 in ($TMP2) DO (IF "%%1"=="%%a" SET "MCH=1")
IF NOT "%%a"=="" IF NOT DEFINED MCH ECHO.%%a>>"%CACHE_FOLDER%\%NEW_NAME%_%TGT_LST%.LST")
IF EXIST "$TMP*" DEL /F $TMP*>NUL
EXIT /B
:LIST_BASE_CREATE
CLS&&CALL:PAD_LINE&&ECHO                               File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&SET "LIST_INSERTA= [ @ ]\[Current-Environment]"
...
CALL:IF_LIVE2
DISM /%APPLY_TARGET% /GET-CURRENTEDITION>"$INFO"
SET "INFO_E="&&SET "INFO_V="&&FOR /F "TOKENS=1-9 DELIMS=: " %%a in ($INFO) DO (
...
CALL:PAD_LINE&&ECHO Getting appx listing..&&DEL /F "$INFO">NUL 2>&1
...
REG EXPORT "%HIVE_SOFTWARE%\Microsoft\Windows\CurrentVersion\Component Based Servicing\Packages" $REG3 >NUL 2>&1
...
ECHO [%BASETMP% DONE]&&CALL:PAD_LINE&&ECHO Getting service listing..
...
SET "BASE_TYPE="&&CALL:PAD_LINE&&ECHO                          End of List Creation&&CALL:PAD_LINE&&CALL:TITLECARD
EXIT /B
...
EXIT /B
:LIST_PACK_CREATE
CLS&&CALL:PAD_LINE&&ECHO                               File Picker&&SET "LIST_FMT=PLAIN"&&CALL:FILE_LIST>NUL
...
EXIT /B
:IMAGEMGR_RUN_LIST
CLS&&CALL:PAD_LINE&&ECHO                               File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.LST"&&ECHO   AVAILABLE LST'S:&&CALL:FILE_LIST
...
IF "%VHDX_BACKUP%"=="COPY" ECHO.&&ECHO Copying %IMAGE_FOLDER%\%SELECT%...&&COPY /Y "%IMAGE_FOLDER%\%SELECT%" "%IMAGE_FOLDER%\Copy_%SELECT%"&&SET "SELECT=Copy_%SELECT%"&&ECHO.
SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
...
IF "%%a"=="SERVICES-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SC DELETE $BRUTE>NUL 2>&1
...
SET "NORESTART="&&IF DEFINED LIVE_APPLY SET "NORESTART=/NORESTART "
...
IF "%SANDWICH_LAYER%"=="RUN-ONCE" ECHO Copying Package [%SANDWICH_MEAT%] for RunOnce...&&CALL:PAD_LINE&&IF EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" ECHO {%SANDWICH_MEAT%}>>"%APPLYDIR_MASTER%\$\RUNONCE.LST"
IF "%SANDWICH_LAYER%"=="RUN-ONCE" IF EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" COPY /Y "%PACK_FOLDER%\%SANDWICH_MEAT%" "%APPLYDIR_MASTER%\$">NUL
IF "%SANDWICH_LAYER%"=="RUN-ONCE" IF NOT EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" ECHO %PACK_FOLDER%\%SANDWICH_MEAT% is missing&&CALL:PAD_LINE
IF "%SANDWICH_LAYER%"=="SETUP-COMPLETE" ECHO Copying Package [%SANDWICH_MEAT%] for SetupComplete...&&CALL:PAD_LINE&&IF EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" ECHO {%SANDWICH_MEAT%}>>"%APPLYDIR_MASTER%\$\SETUPCOMPLETE.LST"
IF "%SANDWICH_LAYER%"=="SETUP-COMPLETE" IF EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" COPY /Y "%PACK_FOLDER%\%SANDWICH_MEAT%" "%APPLYDIR_MASTER%\$">NUL
IF "%SANDWICH_LAYER%"=="SETUP-COMPLETE" IF NOT EXIST "%PACK_FOLDER%\%SANDWICH_MEAT%" ECHO %PACK_FOLDER%\%SANDWICH_MEAT% is missing&&CALL:PAD_LINE
...
IF "%SANDWICH_LAYER%"=="FEATURES-ENABLE" CALL:IF_LIVE2&&DISM /%APPLY_TARGET% %NORESTART%/ENABLE-FEATURE /PACKAGENAME:"%SANDWICH_MEAT%" /ALL>$DISM1
IF "%SANDWICH_LAYER%"=="FEATURES-ENABLE" SET "DISMSG="&&FOR /F "TOKENS=1 DELIMS=." %%a in ($DISM1) DO (SET "DISMSG="&&IF "%%a"=="The operation completed successfully" CALL SET "DISMSG=%%a"&&CALL ECHO                 [%%a]&&CALL:PAD_LINE)
IF "%SANDWICH_LAYER%"=="FEATURES-DISABLE" CALL:IF_LIVE2&&ECHO Disabling Feature [%SANDWICH_MEAT%]...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% %NORESTART%/DISABLE-FEATURE /FEATURENAME:"%SANDWICH_MEAT%" /REMOVE>$DISM1
IF "%SANDWICH_LAYER%"=="FEATURES-DISABLE" SET "DISMSG="&&FOR /F "TOKENS=1 DELIMS=." %%a in ($DISM1) DO (SET "DISMSG="&&IF "%%a"=="The operation completed successfully" CALL SET "DISMSG=%%a"&&CALL ECHO                 [%%a]&&CALL:PAD_LINE)
IF "%SANDWICH_LAYER%"=="FEATURES-ENABLE" IF NOT DEFINED DISMSG CALL ECHO                  [Feature stub or unable to remove]&&CALL:PAD_LINE
IF "%SANDWICH_LAYER%"=="FEATURES-DISABLE" IF NOT DEFINED DISMSG CALL ECHO                  [Feature stub or unable to remove]&&CALL:PAD_LINE
...
IF "%SANDWICH_LAYER%"=="PACKAGES-DELETE" SET "COMP_CNT="&&FOR /F "TOKENS=1* DELIMS=:~" %%a IN ($REG1) DO (IF NOT "%%a"=="" CALL SET /A COMP_CNT+=1&&CALL SET "TX1=%%a"&&CALL SET "TX2=%%b"&&CALL SET "TARGET_MEAT=%SANDWICH_MEAT%"&&CALL:COMP_HUNT)
IF "%SANDWICH_LAYER%"=="APPX-DELETE" CALL:APPX_HUNT
IF "%SANDWICH_LAYER%"=="SERVICES-DELETE" CALL:SVC_HUNT
IF "%SANDWICH_LAYER%"=="TASKS-DELETE" CALL:TASK_HUNT
...
SET "NULL_CNT="&&FOR /F "TOKENS=1* DELIMS=:~" %%1 IN ($REG2) DO (IF NOT "%%1"=="" CALL SET /A NULL_CNT+=1&&CALL SET "X1=%%1"&&CALL SET "X2=%%2"&&CALL:COMPBBQ)
...
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_MIX
DISM /%APPLY_TARGET% %NORESTART%/REMOVE-PACKAGE /PACKAGENAME:"%TARGET_MEAT%~%X2%" >$DISM1
SET "DISMSG="&&FOR /F "TOKENS=1 DELIMS=." %%1 in ($DISM1) DO (SET "DISMSG="&&IF "%%1"=="The operation completed successfully" CALL SET DISMSG=%%1)
...
EXIT /B
:TASK_HUNT
CALL:IF_LIVE1&&REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tree\%SANDWICH_MEAT%" /F Id /c /e /s>$REG1 2>&1
...
IF NOT DEFINED TASK_GO SET "REGMSG=Task [%SANDWICH_MEAT%] doesn't exist"&&GOTO:TASK_SKIP
SET "TASK_CNT="&&CALL SET "X1LAST="&&FOR /F "TOKENS=1-9* DELIMS={}:" %%a IN ($REG1) DO (IF NOT "%%a"=="" CALL SET /A TASK_CNT+=1&&CALL SET "X1=%%a"&&CALL SET "X2=%%b"&&CALL SET "TARGET_MEAT=%SANDWICH_MEAT%"&&CALL:TASKBBQ&&CALL:TASK_CHK)
:TASK_SKIP
...
EXIT /B
:TASK_CHK
REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tree\%SANDWICH_MEAT%" /F Id /c /e /s>$REG2 2>&1
...
:TASKBBQ
IF "%X1%"=="%ENDQ%" EXIT /B
IF "%TARGET_MEAT%"=="%X1LAST%" EXIT /B
SET "REGMSG="&&IF NOT "%TARGET_MEAT%"=="%X1LAST%" ECHO Deleting Task [%TARGET_MEAT%]...&&CALL:PAD_LINE&&SET "X1LAST=%TARGET_MEAT%"
IF "%TASK_CNT%"=="1" IF "%X1%"=="ERROR" SET "REGMSG=Task [%TARGET_MEAT%] doesn't exist"&&CALL:PAD_LINE
IF "%TASK_CNT%"=="1" IF "%X1%"=="%ENDQ%" SET "REGMSG=Task [%TARGET_MEAT%] doesn't exist"&&CALL:PAD_LINE
...
EXIT /B
:SVC_HUNT
CALL:IF_LIVE1&&FOR %%1 in (%PROBLEM_SVC%) DO (IF "%SANDWICH_MEAT%"=="%%1" CALL ECHO Deleting Service [%SANDWICH_MEAT%]...&&CALL:PAD_LINE&&CALL ECHO             [The operation did NOT complete successfully]&&CALL:PAD_LINE&&EXIT /B)
REG QUERY "%HIVE_SYSTEM%\ControlSet001\Services\%SANDWICH_MEAT%" /F ImagePath /c /e /s>$REG1 2>&1
IF EXIST "%PROG_SOURCE%\$BRUTE.CMD" DEL /F "%PROG_SOURCE%\$BRUTE.CMD">NUL
SET "SVC_GO="&&SET "SVC_CNT="&&FOR /F "TOKENS=1-9* DELIMS=: " %%a IN ($REG1) DO (IF "%%a"=="ImagePath" IF NOT "%%c"=="NUL" CALL SET "SVC_GO=1")
IF NOT DEFINED SVC_GO SET "REGMSG=Service [%SANDWICH_MEAT%] doesn't exist"&&GOTO:SVC_SKIP
SET "SVC_CNT="&&CALL SET "X1LAST="&&FOR /F "TOKENS=1-9* DELIMS={}:" %%a IN ($REG1) DO (IF NOT "%%a"=="" CALL SET /A SVC_CNT+=1&&CALL SET "X1=%%a"&&CALL SET "X2=%%b"&&CALL SET "TARGET_MEAT=%SANDWICH_MEAT%"&&CALL:SVCBBQ&&CALL:NULL)
REG QUERY "%HIVE_SYSTEM%\ControlSet001\Services\%SANDWICH_MEAT%" /F ImagePath /c /e /s>$REG2 2>&1
...
IF "%X1%"=="%ENDQ%" EXIT /B
IF "%TARGET_MEAT%"=="%X1LAST%" EXIT /B
SET "X1LAST=%TARGET_MEAT%"&&SET "REGMSG="&&ECHO Deleting Service [%TARGET_MEAT%]...&&CALL:PAD_LINE
IF "%SVC_CNT%"=="1" IF "%X1%"=="ERROR" SET "REGMSG=Service [%TARGET_MEAT%] doesn't exist"&&CALL:PAD_LINE
IF "%SVC_CNT%"=="1" IF "%X1%"=="%ENDQ%" SET "REGMSG=Service [%TARGET_MEAT%] doesn't exist"&&CALL:PAD_LINE
IF DEFINED REGMSG EXIT /B
...
IF "%PROG_MODE%"=="RAMDISK" IF "%BRUTE_FORCE%"=="ENABLED" IF "%FILE_HEADER%"=="SERVICES-DELETE" SET "BRUTE_FORCE="&&SET "BRUTE_FORCE_TMP=1"
IF "%PROG_MODE%"=="RAMDISK" IF "%BRUTE_FORCE%"=="ENABLED" IF "%FILE_HEADER%"=="TASKS-DELETE" SET "BRUTE_FORCE="&&SET "BRUTE_FORCE_TMP=1"
IF "%FILE_HEADER%"=="TASKS-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
IF "%FILE_HEADER%"=="SERVICES-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
...
IF NOT EXIST "%PACK_FOLDER%\%%1" ECHO %PACK_FOLDER%\%%1 is missing&&CALL:PAD_LINE)
...
IF "%FILE_HEADER%"=="SERVICES-DELETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($LST) DO (CALL SET "SANDWICH_MEAT=%%1"&&CALL:SVC_HUNT)
IF "%FILE_HEADER%"=="FEATURES-ENABLE" CALL:IF_LIVE2&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($LST) DO (CALL ECHO Enabling Feature [%%1]...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% %NORESTART%/ENABLE-FEATURE /PACKAGENAME:"%%1" /ALL>$DISM1
SET "DISMSG="&&FOR /F "TOKENS=1 DELIMS=." %%a in ($DISM1) DO (SET "DISMSG="&&IF "%%a"=="The operation completed successfully" CALL SET "DISMSG=%%a"&&CALL ECHO                 [%%a]&&CALL:PAD_LINE&&DEL /F $DISM1>NUL)
IF NOT DEFINED DISMSG CALL ECHO                   [Feature stub or unable to remove]&&CALL:PAD_LINE)
IF "%FILE_HEADER%"=="FEATURES-DISABLE" CALL:IF_LIVE2&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($LST) DO (CALL ECHO Disabling Feature [%%1]...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% %NORESTART%/DISABLE-FEATURE /FEATURENAME:"%%1" /REMOVE>$DISM1
SET "DISMSG="&&FOR /F "TOKENS=1 DELIMS=." %%a in ($DISM1) DO (SET "DISMSG="&&IF "%%a"=="The operation completed successfully" CALL SET "DISMSG=%%a"&&CALL ECHO                 [%%a]&&CALL:PAD_LINE&&DEL /F $DISM1>NUL)
IF NOT DEFINED DISMSG CALL ECHO                   [Feature stub or unable to remove]&&CALL:PAD_LINE)
IF "%FILE_HEADER%"=="APPX-DELETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($LST) DO (CALL SET "SANDWICH_MEAT=%%1"&&CALL:APPX_HUNT)
...
SET "COMP_CNT="&&FOR /F "TOKENS=1* DELIMS=:~" %%a IN ($REG1) DO (IF NOT "%%a"=="" CALL SET /A COMP_CNT+=1&&CALL SET "TX1=%%a"&&CALL SET "TX2=%%b"&&CALL SET "TARGET_MEAT=%%1"&&CALL:COMP_HUNT))
IF "%FILE_HEADER%"=="TASKS-DELETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($LST) DO (CALL SET "SANDWICH_MEAT=%%1"&&CALL:TASK_HUNT)
IF EXIST "%PROG_SOURCE%\ScratchPack" DISM /cleanup-MountPoints>NUL 2>&1
IF EXIST "%PROG_SOURCE%\ScratchPack" ATTRIB -R -S -H "%PROG_SOURCE%\ScratchPack" /S /D /L>NUL 2>&1
IF EXIST "%PROG_SOURCE%\ScratchPack" RD /S /Q "\\?\%PROG_SOURCE%\ScratchPack">NUL 2>&1
IF "%BRUTE_FORCE%"=="ENABLED" SC DELETE $BRUTE>NUL 2>&1
IF DEFINED BRUTE_FORCE_TMP SET "BRUTE_FORCE=ENABLED"&&SET "BRUTE_FORCE_TMP="
IF EXIST "$DISM*" DEL /F "$DISM*">NUL 2>&1
...
ECHO  {1}RestoreHealth&&ECHO  {2}Cleanup&&ECHO  {3}ResetBase&&ECHO  {4}SPSuperseded&&SET "PROMPT_SET=DISM_MENU"&&CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:PROMPT_SET
...
EXIT /B
:IMAGEMGR_INSPECT
CLS&&CALL:PAD_LINE&&ECHO                               File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&SET "LIST_INSERTA= [ @ ]\[Current-Environment]"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                        Enter File # To Inspect&&CALL:PAD_LINE&&CALL:PAD_PREV&&CALL:MENU_SELECT&&CLS
...
CALL:PAD_LINE&&ECHO                          Windows Source Info&&CALL:PAD_LINE&&CALL:SCRATCHDIR_CREATE&&SET "VDISK=%IMAGE_FOLDER%\%TARGET_IMAGE%"&&CALL:VDISK_ATTACH
...
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tasks" /f Path /c /e /s>"$TSK"
REG QUERY "%HIVE_SYSTEM%\ControlSet001\Services" /f Type /c /e /s >"$VC"
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
...
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
SET "SVC_CNT="&&FOR /F "TOKENS=1-9 DELIMS=\ " %%a in ($VC) DO (
...
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
SET "TASK_CNT="&&FOR /F "TOKENS=1* DELIMS=\" %%a in ($TSK) DO (IF "%%a"=="    Path    REG_SZ    " SET "TASK_NAME=%%b"&&CALL:FORENSICS_TASKS_QUERY)
...
:IMAGEPROC_START
REM IMAGEPROC_IMAGEPROC_IMAGEPROC_IMAGEPROC_IMAGEPROC_IMAGEPROC_IMAGEPROC_IMAGEPROC_
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&CLS&&CALL:TITLEGEN&&CALL:PAD_LINE&&ECHO                            Image Processing&&CALL:PAD_LINE&&IF NOT DEFINED WIM_COMPRESS SET "WIM_COMPRESS=FAST"
...
IF NOT DEFINED PROG_OPER CALL:PAD_LINE
IF DEFINED PROG_OPER CALL:PAD_LINE&&ECHO                    Enter File # To [%PROG_OPER%] {/}Type&&CALL:PAD_LINE&&CALL:TITLECARD
CALL:PAD_PREV&&CALL:MENU_SELECT&&IF DEFINED PROG_OPER CALL:FILE_LIST>NUL
IF NOT DEFINED PROG_OPER IF NOT DEFINED SELECT GOTO:PROG_MAIN
...
Dism /Get-ImageInfo /ImageFile:"%IMAGE_FOLDER%\%WIM_SOURCE_IMAGE%" /Index:%WIM_INDEX_IMAGE% >"$INFO"
FOR /F "tokens=1-5 delims=<> " %%a in ($INFO) DO (IF "%%a"=="Edition" SET "WIM_DESC_IMAGE=%%c")
FOR /F "tokens=1-5 delims=<> " %%a in ($INFO) DO (IF "%%a"=="Languages" IF NOT "%%c"=="" SET "WIM_SOURCE_IMAGE=-SELECT-"&&SET "WIM_DESC_IMAGE=NULL")
DEL /Q /F "$INFO">NUL
EXIT /B
...
:DISKMGR_START
REM DISKMGR_DISKMGR_DISKMGR_DISKMGR_DISKMGR_DISKMGR_DISKMGR_
@ECHO OFF&&SET "DISK_LETTER="&&SET "DISK_MSG="&&CALL:COLOR_CHK&&CLS&&CALL:TITLEGEN&&CALL:PAD_LINE&&ECHO                             Disk Management&&CALL:PAD_LINE&&IF DEFINED PROG_OPER CALL:TITLECARD
...
:FILEMGR_START
REM FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_
@ECHO OFF&&CALL:COLOR_CHK&&CLS&&CALL:TITLEGEN&&CALL:PAD_LINE&&ECHO                             File Management&&CALL:PAD_LINE
...
SET "FILEMGR_SOURCE=%FILEMGR_TARGET%"&&SET "FILEMGR_TARGET=%FILEMGR_SOURCE%"
EXIT /B
REM $ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP
:$ETUP_START
REM $ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP_$ETUP
CLS&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:TITLEGEN&&CALL:COLOR_CHK
...
EXIT /B
:BOOTWIM_INDEX
IF EXIST "%IMAGEFILE_BOOT%" Dism /Get-ImageInfo /ImageFile:"%IMAGEFILE_BOOT%" /Index:3>$INFO
IF EXIST "%IMAGEFILE_BOOT%" FOR /F "tokens=5 skip=5 delims=<> " %%a in ($INFO) DO (IF "%%a"=="Setup" SET "BOOTWIM_INDEX=3"&&GOTO:WIM_ESCAPE)
IF EXIST "%IMAGEFILE_BOOT%" Dism /Get-ImageInfo /ImageFile:"%IMAGEFILE_BOOT%" /Index:2>$INFO
IF EXIST "%IMAGEFILE_BOOT%" FOR /F "tokens=5 skip=5 delims=<> " %%a in ($INFO) DO (IF "%%a"=="Setup" SET "BOOTWIM_INDEX=2"&&GOTO:WIM_ESCAPE)
IF EXIST "%IMAGEFILE_BOOT%" Dism /Get-ImageInfo /ImageFile:"%IMAGEFILE_BOOT%" /Index:1>$INFO
IF EXIST "%IMAGEFILE_BOOT%" FOR /F "tokens=5 skip=5 delims=<> " %%a in ($INFO) DO (IF "%%a"=="Setup" SET "BOOTWIM_INDEX=1"&&GOTO:WIM_ESCAPE)
:WIM_ESCAPE
IF NOT DEFINED BOOTWIM_INDEX SET "BOOTWIM_INDEX=1"
DEL /Q /F "$INFO">NUL 2>&1
EXIT /B
:BCD_CREATE
...
:MAKER_START
REM MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_MAKER_
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&CLS&&CALL:TITLEGEN&&CALL:PAD_LINE&&ECHO                          $PK Package Creator&&CALL:PAD_LINE
IF NOT DEFINED MAKER_SLOT SET "MAKER_SLOT=1"
...
IF "%SELECT%"=="C" IF NOT "%PackType%"=="NULL" SET "PROG_OPER=MAKER_CREATE"
