...
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&CHCP 437>NUL&&SET $VER_CUR=1116&&SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&Reg.exe query "HKU\S-1-5-19\Environment">NUL
...
SET "CORRUPT_FLAG="&&IF NOT EXIST "%MLB%\*.MST" ECHO [%SELECT%] IS CORRUPT&&SET "CORRUPT_FLAG=1"&&RD /Q /S "\\?\%MLB%">NUL 2>&1
