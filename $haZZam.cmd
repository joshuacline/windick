...
IF "%FILE_HEADER%"=="FEATURES-ENABLE" CALL:TEXTCOLOR&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (DISM /%APPLY_TARGET% /ENABLE-FEATURE /PACKAGENAME:"%%1" /ALL)
IF "%FILE_HEADER%"=="FEATURES-DISABLE" CALL:TEXTCOLOR&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Disabling Feature [%%1] ...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% /DISABLE-FEATURE /FEATURENAME:"%%1" /REMOVE)
IF "%FILE_HEADER%"=="APPX-DELETE" CALL:TEXTCOLOR&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Removing AppX Package [%%1] ...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% /REMOVE-Provisionedappxpackage /PACKAGENAME:"%%1")
...
IF "%FILE_HEADER%"=="PACKAGES-DELETE" CALL:TEXTCOLOR&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Removing Component [%%1] ...&&CALL:PAD_LINE
...
IF "%SELECT%"=="1" SET PACK_CONFIG=%PACK_CFG_1%&&SET PackName=VBS_Enable&&SET PackDesc=Enable Link-Layer-Topology Discovery Responder Driver
IF "%SELECT%"=="2" SET PACK_CONFIG=%PACK_CFG_2%&&SET PackName=VBS_Disable&&SET PackDesc=Disable Link-Layer-Topology Discovery Responder Driver
CALL:PACK_CONFIG
...
EXIT /B
:PACKEX_DEBUG
ECHO  - Virtualization Based Security -&&ECHO {1}Pause&&ECHO {2}Echo On {3}Echo Off&&CALL:MENU_SELECT
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
