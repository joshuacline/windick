...
IF "%FILE_HEADER%"=="RUN-ONCE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for SetupComplete Prep...&&CALL:PAD_LINE&&COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
IF "%FILE_HEADER%"=="SETUP-COMPLETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for RunOnce Prep...&&CALL:PAD_LINE&&COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
...
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
ECHO COMPONENT STORE SCAN>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
...
CALL:PAD_LINE&&ECHO                       *Getting Package Info...*&&CALL:PAD_LINE&&CALL:TITLECARD&&SET /A TEXTCOLOR+=1&&CALL:TEXTCOLOR
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
ECHO IMAGE PACKAGE LIST>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
