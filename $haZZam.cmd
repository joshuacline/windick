...
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&SET $VER_CUR=1109
...
FOR /F "TOKENS=1-9 DELIMS=:~ " %%a in ($TMP.TXT) DO (IF "%%a %%b"=="Package Identity" CALL SET "BASECAP=%%c"&&CALL SET "BASEPRE=%%c"&&CALL:NULL&&CALL:BASECAP)
...
IF "%SANDWICH_LAYER%"=="PACKAGES-DELETE" CALL SET "TARGET_MEAT=%SANDWICH_MEAT%"&&CALL:COMP_FINDER
EXIT /B
:COMP_FINDER
SET "TEMP1X="&&SET "TEMP2X="&&SET "ENDQ=End of search"
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows\CurrentVersion\Component Based Servicing\Packages" /F "%TARGET_MEAT%">$TMP
SET COMP_CNT=&&FOR /F "TOKENS=1* DELIMS=:~" %%a IN ($TMP) DO (IF NOT "%%a"=="" CALL SET /A COMP_CNT+=1&&CALL SET "TEMP1X=%%a"&&CALL SET "TEMP2X=%%b"&&CALL:WTFBBQ)
IF EXIST "$TMP" DEL /F "$TMP">NUL
EXIT /B
:WTFBBQ
IF "%COMP_CNT%"=="1" IF "%TEMP1X%"=="%ENDQ%" CALL:PAD_LINE&&ECHO Component [%TARGET_MEAT%] doesn't exist
IF "%TEMP1X%"=="%ENDQ%" EXIT /B
CALL:PAD_LINE&&ECHO  Removing Component [%TARGET_MEAT%~%TEMP2X%] ...&&CALL:PAD_LINE
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
REG ADD "%TEMP1X%~%TEMP2X%" /V "Visibility" /T REG_DWORD /D "1" /F>NUL 2>&1
REG DELETE "%TEMP1X%~%TEMP2X%\Owners" /F>NUL 2>&1
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_MIX
DISM /%APPLY_TARGET% /REMOVE-PACKAGE /PACKAGENAME:"%TARGET_MEAT%~%TEMP2X%" >$DISM
SET "DISMSG="&&IF EXIST "$DISM" FOR /F "TOKENS=1-9* DELIMS=" %%1 in ($DISM) DO (SET "DISMSG="&&IF "%%1"=="The operation completed successfully." CALL SET DISMSG=%%1%%2%%3%%4)
IF EXIST "$DISM" IF NOT DEFINED DISMSG ECHO                 [Component leftover or unable to remove]
...
IF "%FILE_HEADER%"=="PACKAGES-DELETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in ($TMP.TXT) DO (CALL SET "TARGET_MEAT=%%1"&&CALL:COMP_FINDER)
IF EXIST "%PROG_SOURCE%\ScratchPack" DISM /cleanup-MountPoints>NUL 2>&1
IF EXIST "%PROG_SOURCE%\ScratchPack" ATTRIB -R -S -H "%PROG_SOURCE%\ScratchPack" /S /D /L>NUL 2>&1
IF EXIST "%PROG_SOURCE%\ScratchPack" RD /S /Q "\\?\%PROG_SOURCE%\ScratchPack">NUL 2>&1
