...
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&SET $VER_CUR=1108
...
CLS&&CALL:TITLEGEN&&CALL:PAD_LINE
...
IF DEFINED MARK IF "%ROW%"=="%ROW_TGT%" CALL SET FS_LAST=[SKP[%SKIPPER%] [DLM[%DELIMS%] [IN[%CUR_CMD%] [IF[%%%%%CLM_TGT%]==[%CLM_DAT%]
...
REM $ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS_$ETTINGS
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK
CLS&&CALL:TITLEGEN&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF NOT DEFINED PROG_OPER IF NOT DEFINED COLOR_SLOT ECHO    {-}Text Color   [SHUFFLE]
IF NOT DEFINED PROG_OPER IF DEFINED COLOR_SLOT ECHO    {-}Text Color   [%COLOR_SLOT%]
...
SET "CAME_FROM="&&SET "BTMP=%WINDIR%\System32\config\ELAM"&&CALL:TITLEGEN&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
:FILEMGR_START
REM FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_FILEMGR_
@ECHO OFF&&CALL:COLOR_CHK&&CLS&&CALL:TITLEGEN&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
@ECHO OFF&&CLS&&CALL:PAD_LINE&&ECHO  FOR~SIGHT - WANNA ARGUE?&&CALL:PAD_LINE&&CALL:CRASHY
IF NOT DEFINED FOR_SAV SET "FOR_SAV=FRESH"&&SET "CLM_TGT=1"&&SET "CMD_MODE=INT"&&SET "GET_ROW=1"
IF DEFINED CUR_CMD SET "ROW="&&IF EXIST "$FOR" FOR /F "TOKENS=1-9 DELIMS=<>()" %%A IN ($FOR) DO (SET "ARGUE=%%A%%B%%C%%D%%E%%F%%G%%H"&&CALL:ARGUE)
IF EXIST "$FOR" SET MARK=&&DEL /F $FOR>NUL 2>&1
IF DEFINED FS_LAST ECHO  {SAVED} %FS_LAST%
CALL:PAD_LINE&&ECHO  {T}%CLM_TGT%] {F}ull  {R}ef  {V}iew     {G}o          {M}ode[%FOR_SAV%]  {Q}uit&&CALL:PAD_LINE
IF "%CMD_MODE%"=="EXT" ECHO  {D}ELIMS[%DELIMS%]  {C}MD[EXT.CMD[%CUR_CMD%] 
IF "%CMD_MODE%"=="INT" ECHO  {D}ELIMS[%DELIMS%]  {C}MD [%CUR_CMD%]
CALL:PAD_LINE&&ECHO                  Press (Enter) to reparse FOR results&&CALL:MENU_SELECT
IF "%SELECT%"=="Q" EXIT /B
...
IF "%SELECT%"=="C" IF "%CMD_MODE%"=="EXT" SET "SELECT="&&SET "CMD_MODE=INT"&&SET "PROMPT_SET=CUR_CMD"&&CALL:PAD_LINE&&ECHO              ENTER A COMMAND WITHOUT QUOTES/SPECIAL CHARS&&CALL:PAD_LINE&&CALL:PROMPT_SET_ANY&&GOTO:FOR_SIGHT
IF "%SELECT%"=="C" IF "%CMD_MODE%"=="INT" IF NOT EXIST EXT.CMD ECHO;REG QUERY "HKCU\Control Panel\International\User Profile">EXT.CMD
IF "%SELECT%"=="C" IF "%CMD_MODE%"=="INT" IF EXIST EXT.CMD SET "SELECT="&&SET "CMD_MODE=EXT"&&SET /P CUR_CMD=<EXT.CMD&&GOTO:FOR_SIGHT
