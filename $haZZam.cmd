...
:IMAGEMGR_START
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
@ECHO OFF&&CLS&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&CALL:TITLEGEN&&SET "PROBLEM_PACK=EDGEUPDATE EDGEUPDATEM WDNISSVC WINDEFEND WMPNETWORKSVC"&&SET "LIVE_APPLY="&&SET "SANDWICH_PARSE="&&SET "LOL_PARSE="&&SET "LIST_CREATE="&&SET "FILE_HEADER="&&SET "HEADER_TGT="&&SET "HEADER_TGT_LOL="&&SET "ERR_MSG="&&SET "TARGET_IMAGE="&&CALL:PAD_LINE&&ECHO                            Image Management&&CALL:PAD_LINE
...
EXIT /B
:IMAGEMGR_LIST_MAIN
CLS&&CALL:PAD_LINE&&ECHO                         $haZZam^^! List Creator&&CALL:PAD_LINE&&ECHO.&&ECHO  [ C ]\[ImageApply[CAB]&&ECHO  [ M ]\[ImageApply[MSU]&&ECHO  [ I ]\[ImageApply[$PK]&&ECHO  [ S ]\[SetupComplete[$PK]&&ECHO  [ R ]\[RunOnce[$PK]&&ECHO.&&CALL:PAD_LINE&&ECHO  [ - ]\[List-of-Lists]&&ECHO  [ + ]\[Sandwich-List]&&CALL:PAD_LINE&&SET "LIST_INSERTA= [ * ]\[Create List-Base[MLB]"
...
EXIT /B
:SVC_HUNT
CALL:IF_LIVE1&&FOR %%1 in (%PROBLEM_PACK%) DO (IF "%SANDWICH_MEAT%"=="%%1" CALL ECHO Deleting Service [%SANDWICH_MEAT%]...&&CALL:PAD_LINE&&CALL ECHO             [The operation did NOT complete successfully]&&CALL:PAD_LINE&&EXIT /B)
