...
IF "%OBJ_IMAGEMGR%"=="WIM" CLS&&CALL:PAD_LINE&&ECHO                           Extracting image....&&CALL:PAD_LINE&&CALL:TITLECARD&&SET "DISK_LETTER=Z"&&CALL:VDISK_CREATE
IF "%OBJ_IMAGEMGR%"=="WIM" DISM /APPLY-IMAGE /IMAGEFILE:"%IMAGE_FOLDER%\%SELECT%" /INDEX:%INDEX_TEMP% /APPLYDIR:"Z:"
IF "%OBJ_IMAGEMGR%"=="VHDX" SET INDEX_TEMP=1&&SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&SET "DISK_LETTER=Z"&&CALL:VDISK_ATTACH
:THE_ACTION
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
...
IF NOT DEFINED PROG_OPER IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" ECHO     {M}ode[%BCD_SYSTEM%]                 {G}o^^!     ~ {V} Copy %VHDX_SOURCE_$ETUP%} ~ &&CALL:PAD_LINE
IF DEFINED PROG_OPER ECHO                  Enter File # To [%PROG_OPER%] {/}Type&&CALL:PAD_LINE
ECHO                Press (Enter) to Return to Previous Menu
CALL:MENU_SELECT&&IF DEFINED PROG_OPER CALL:FILE_LIST>NUL
IF NOT DEFINED PROG_OPER IF NOT DEFINED SELECT GOTO:PROG_MAIN
...
IF "%CAME_FROM%"=="$ETUP" IF "%BCD_SYSTEM%"=="SLOT" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" ECHO COPYING %VHDX_SOURCE_$ETUP%...&&COPY /Y "\\?\%IMAGE_FOLDER%\%VHDX_SOURCE_$ETUP%" "%LAST_LETTER%:\$\1.VHDX">NUL 2>&1
