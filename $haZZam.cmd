...
EXIT /B
:IMAGEMGR_RUN_LIST
SET "LIVE_APPLY="&&SET "IMAGE_TYPE_CREATE="&&SET "FILE_HEADER="&&SET "LOL_PARSE="&&CLS&&CALL:PAD_LINE&&ECHO                      $-h-@-Z-Z-@-m^^! File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.LST"&&ECHO   AVAILABLE LST'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                     Enter File # To Select A List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" EXIT /B
SET "HEADER_TGT=%CACHE_FOLDER%\%SELECT%"&&CALL:HEADER_PARSE
...
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
IF DEFINED RUNFLAG IF NOT "%FILE_HEADER%"=="LIST-OF-LISTS" CALL:IMAGEMGR_LIST_OPER
IF DEFINED RUNFLAG IF "%FILE_HEADER%"=="LIST-OF-LISTS" CALL:LOL_RUNNER
...
EXIT /B
:LOL_RUNNER
FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%a in (%HEADER_TGT%) DO (IF EXIST "%CACHE_FOLDER%\%%a" CALL SET "LOL_PARSE=1"&&CALL SET "HEADER_TGT_TMP=%CACHE_FOLDER%\%%a"&&CALL:HEADER_PARSE_TMP)
EXIT /B
:HEADER_PARSE_TMP
SET "RUNFLAG="&&SET /P FILE_HEADER=<"%HEADER_TGT_TMP%"
...
IF DEFINED LOL_PARSE COPY /Y "%HEADER_TGT_TMP%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
IF NOT DEFINED LOL_PARSE COPY /Y "%HEADER_TGT%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
IF "%FILE_HEADER%"=="RUN-ONCE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for RunOnce Prep...&&CALL:PAD_LINE&&IF NOT "%%1"=="" IF EXIST "%PACK_FOLDER%\%%1" COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
IF "%FILE_HEADER%"=="SETUP-COMPLETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for SetupComplete Prep...&&CALL:PAD_LINE&&IF NOT "%%1"=="" IF EXIST "%PACK_FOLDER%\%%1" COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
...
SET "FILE_HEADER="&&SET "HEADER_TGT_TMP="
CALL:MOUNT_INT
EXIT /B
...
IF DEFINED LOL_PARSE COPY /Y "%HEADER_TGT_TMP%" "%APPLYDIR_MASTER%\$\SETUPCOMPLETE.LST">NUL 2>&1
...
IF NOT EXIST "%APPLYDIR_MASTER%\$" MD "%APPLYDIR_MASTER%\$">NUL 2>&1
COPY /Y "%PROG_FOLDER%\$HAZZAM.CMD" "%APPLYDIR_MASTER%\$">NUL 2>&1
IF DEFINED LOL_PARSE COPY /Y "%HEADER_TGT_TMP%" "%APPLYDIR_MASTER%\$\RUNONCE.LST">NUL 2>&1
...
IF DEFINED DISK_MSG ECHO %DISK_MSG%&&ECHO.&&SET "DISK_MSG="
...
CALL:PART_UNMOUNT
CALL:DISKMGR_ERASE
SET "EFI_TYPE=NML"&&SET /A "PART_CNT+=1"
...
(ECHO.select disk %DISK_NUMBER%&&ECHO.select partition 2&&ECHO.assign letter=S noerr&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART"
IF EXIST "Q:\" IF EXIST "S:\" EXIT /B
IF "%PART_CNT%"=="4" ECHO The drive is currently in use or incompatible. Continue to try?&&ECHO  {Z}Continue {Enter}Abort&&CALL SET "PROMPT_SET=PART_QUIT1"&&CALL:PROMPT_SET
IF NOT "%PART_QUIT1%"=="Z" SET "PART_QUIT=1"
IF "%PART_QUIT1%"=="Z" SET "PART_CNT=0"&&SET "PART_QUIT1="
...
IF EXIST "T:\" (ECHO.select disk %DISK_NUMBER%&&ECHO.clean&&ECHO.convert gpt&&ECHO.select partition 1&&ECHO.delete partition override&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART">NUL 2>&1
...
IF NOT EXIST "%DISK_LETTER%:\" SET "SECOND_GO=1"&&(ECHO.select disk %DISK_NUMBER%&&ECHO.select partition %PART_NUMBER%&&ECHO.Assign letter=%DISK_LETTER% noerr&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART">NUL 2>&1
IF NOT EXIST "%DISK_LETTER%:\" IF DEFINED SECOND_GO SET "SECOND_GO="&&(ECHO.select disk %DISK_NUMBER%&&ECHO.select partition %PART_NUMBER%&&ECHO.Assign letter=%DISK_LETTER% noerr&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART">NUL 2>&1
...
IF NOT EXIST "S:\$" ECHO CAN'T FIND HOME. MAYBE THE DISK-ID HAS CHANGED.ALSO CHECK THE HOME FOLDER NAME&&PAUSE&&EXIT /B
SET "PROG_SOURCE=S:\$"&&SET "PROG_TARGET=S:\$"&&SET "HOME_MOUNT=YES"&&CALL:SETS_LOAD>NUL 2>&1
EXIT /B
:HOME_MANUAL
CALL:DISK_MENU&&CALL:HOME_AUTO>NUL 2>&1
EXIT /B
:REASSIGN_LETTER
CALL:DISK_QUERY>NUL 2>&1
...
IF DEFINED DISK_NUMBER SET "PART_CNT="&&SET "PART_QUIT="&&CALL:PART_CREATE>NUL 2>&1
SET "BOOT_ERROR0=Disk is currently in use - unplug disk - reboot into Windows - replug and try again."
IF NOT EXIST "Q:\" SET "BOOT_ERROR=%BOOT_ERROR0%"&&GOTO:BOOT_CLEANUP
IF NOT EXIST "S:\" SET "BOOT_ERROR=%BOOT_ERROR0%"&&GOTO:BOOT_CLEANUP
IF DEFINED PART_QUIT SET "BOOT_ERROR=%BOOT_ERROR0%"&&GOTO:BOOT_CLEANUP
:BOOT_CREATE
DISM /cleanup-MountPoints>NUL 2>&1
...
IF NOT EXIST "%APPLYDIR_BOOT%\Windows" SET "BOOT_ERROR=MOUNT FAILURE, BOOTWIM_INDEX %BOOTWIM_INDEX%" &&GOTO:BOOT_CLEANUP
...
IF NOT EXIST "Q:\EFI\Boot\bootx64.efi" SET "BOOT_ERROR=EFI MISSING"&&GOTO:BOOT_CLEANUP
CALL:BCD_CREATE>NUL 2>&1
IF NOT EXIST "Q:\EFI\Microsoft\Boot\BCD" SET "BOOT_ERROR=BCD MISSING"&&GOTO:BOOT_CLEANUP
...
CALL:EFI_UNMOUNT>NUL 2>&1
IF DEFINED BOOT_ERROR GOTO:BOOT_ERROR
...
IF "%CAME_FROM%"=="$ETUP" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" IF NOT EXIST "%LAST_LETTER%:\$\*.VHDX" SET "BOOT_ERROR=ERROR COPYING %VHDX_SLOT_$ETUP%.VHDX"
:BOOT_ERROR
IF "%CAME_FROM%"=="$ETUP" ECHO &&SET "CAME_FROM="
IF DEFINED BOOT_ERROR ECHO %BOOT_ERROR%&&ECHO.&&SET "BOOT_ERROR="
...
IF "%BCD_SYSTEM%"=="NAME" SET "BCD_CNT="&&CALL:BCD_VHDX
IF "%BCD_SYSTEM%"=="SLOT" FOR %%a in (9 8 7 6 5 4 3 2 1 0) DO (IF "%%a" LEQ "%BCDSLOT_CNT%" CALL SET "BCD_CNT=%%a"&&CALL:BCD_VHDX)
...
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S11} UAC Prompt Always/Never                             (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S12} NotificationCenter Disable                          (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S13} Net Discovery Enable/Disable                        (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S14} Bluetooth Advertising Enable/Disable                (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S15} Virtualization Based Security Enable/Disable        (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S16} Disable Explorer URL Access                         (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S17} Background Apps Disable                             (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S18} DCOM Enable/Disable (Breaks Stuff)                  (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S19} Prioritize Ethernet                                 (SCRIPTED)
IF "%EXAMPLE_MODE%"=="CREATE" ECHO  {S20} Prioritize WiFi                                     (SCRIPTED)
