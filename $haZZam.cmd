...
EXIT /B
:LOL_RUNNER
FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%a in (%HEADER_TGT%) DO (IF EXIST "%CACHE_FOLDER%\%%a" CALL SET "HEADER_TGT_TMP=%CACHE_FOLDER%\%%a"&&CALL:HEADER_PARSE_TMP)
EXIT /B
:HEADER_PARSE_TMP
...
COPY /Y "%HEADER_TGT_TMP%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
