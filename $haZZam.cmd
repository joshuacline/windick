...
IF NOT DEFINED PROG_OPER ECHO  [LIST]    {C}reate    {V}iew    {G}o^^!        {*}Apply to [%VHDX_BACKUP%-VHDX] &&CALL:PAD_LINE
...
SET "CAME_FROM="&&SET "BTMP=%WINDIR%\System32\config\ELAM"&&TITLE  [%CD%] - $haZZam^^! A native Windows image deployment tool.&&IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF NOT DEFINED PROG_OPER IF "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="ISOLATED" SET "LIST_FMT=PLAIN"&&SET "LIST_DIR=%PROG_SOURCE%\*.VHDX"&&ECHO   BOOT/HOME-FOLDER:&&CALL:FILE_LIST&&CALL:PAD_LINE
IF NOT DEFINED PROG_OPER IF "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="ISOLATED" SET "LIST_FMT=PLAIN"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE
IF NOT DEFINED PROG_OPER IF NOT "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="ISOLATED" SET "LIST_FMT=PLAIN"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&ECHO   AVAILABLE VHDX'S:&&CALL:FILE_LIST&&CALL:PAD_LINE
...
IF NOT DEFINED PROG_OPER IF "%PROG_MODE%"=="RAMDISK" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" ECHO     {R}efreshBoot     ~ { Refresh boot list as [%BCD_SYSTEM%-MODE] } ~&&CALL:PAD_LINE
IF DEFINED PROG_OPER ECHO                  Enter File # To [%PROG_OPER%] {/}Type&&CALL:PAD_LINE
ECHO                Press (Enter) to Return to Previous Menu
CALL:MENU_SELECT&&IF DEFINED PROG_OPER CALL:FILE_LIST>NUL
IF NOT DEFINED PROG_OPER IF NOT DEFINED SELECT GOTO:PROG_MAIN
