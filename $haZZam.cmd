...
@ECHO OFF&&SETLOCAL ENABLEDELAYEDEXPANSION&&SET $VER_CUR=1112
SET "ORIG_CD=%CD%"&&CD /D "%~DP0"&&CHCP 437>NUL
...
IF NOT DEFINED PROG_OPER ECHO  {B}rute TSK/SVC  [%BRUTE_FORCE%]
...
:IMAGEMGR_START
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&CALL:COLOR_CHK&&SET "PROBLEM_PACK=EDGEUPDATE EDGEUPDATEM WDNISSVC WINDEFEND WMPNETWORKSVC"&&SET "LIVE_APPLY="&&SET "SANDWICH_PARSE="&&SET "LOL_PARSE="&&SET "LIST_CREATE="&&SET "FILE_HEADER="&&SET "HEADER_TGT="&&SET "HEADER_TGT_LOL="&&SET "ERR_MSG="&&SET "TARGET_IMAGE="
CLS&&CALL:TITLEGEN&&IF DEFINED PROG_OPER CALL:TITLECARD
...
SET "BRUTE_SVC="&&CALL SET "SANDWICH_LAYER="&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%a in (%HEADER_TGT%) DO (
...
IF "%%a"=="SERVICES-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SET "BRUTE_SVC=1"&&SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
IF "%%a"=="TASKS-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SET "BRUTE_SVC=1"&&SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
...
IF DEFINED BRUTE_SVC SC DELETE $BRUTE>NUL 2>&1
...
SET "BRUTE_SVC="&&IF "%FILE_HEADER%"=="TASKS-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SET "BRUTE_SVC=1"&&SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
SET "BRUTE_SVC="&&IF "%FILE_HEADER%"=="SERVICES-DELETE" IF "%BRUTE_FORCE%"=="ENABLED" (SET "BRUTE_SVC=1"&&SC DELETE $BRUTE>NUL 2>&1
SC CREATE $BRUTE BINPATH="CMD /C START %PROG_SOURCE%\$BRUTE.CMD" START=DEMAND>NUL 2>&1)
