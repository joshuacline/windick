...
ECHO  FOR~SIGHT - WANNA ARGUE?&&CALL:PAD_LINE&&CALL:CRASHY
IF NOT DEFINED FOR_SAV SET FOR_SAV=FRESH&&SET CLM_TGT=1
...
IF NOT DEFINED CUR_CMD SET "CUR_CMD=DIR /B %USERPROFILE%"
IF NOT DEFINED GET_ROW SET "GET_ROW=1"
ECHO  [**] %ROW_DSP% &&CALL:PAD_LINE&&ECHO  {T}%CLM_TGT%] {F}ull  {R}ef  {V}iew     {G}o          {M}ode[%FOR_SAV%]  {Q}uit&&CALL:PAD_LINE
...
IF "%SELECT%"=="E" IF DEFINED ROW_EXT SET "ROW_EXT="&&SET "SELECT="&&SET "CLM_TGT=1"&&GOTO:FOR_SIGHT
...
IF "%SELECT%"=="C" IF "%CUR_CMD%"=="CALL EXT.CMD" SET "SELECT="&&SET "PROMPT_SET=CUR_CMD"&&CALL:PAD_LINE&&ECHO                  ENTER A COMMAND WITHOUT SPECIAL CHARS&&CALL:PAD_LINE&&CALL:PROMPT_SET_ANY
IF "%SELECT%"=="C" IF NOT "%CUR_CMD%"=="CALL EXT.CMD" SET "SELECT="&&SET "CUR_CMD=CALL EXT.CMD"&&IF NOT EXIST EXT.CMD ECHO;REG QUERY "HKCU\Control Panel\International\User Profile">EXT.CMD
