...
IF "%SELECT%"=="RESTART" CALL:RESTART
...
IF EXIST "Z:\" SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH>NUL 2>&1
...
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
REM IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_IMAGEMGR_
@ECHO OFF&&CALL:SETS_HANDLER&&CALL:FOLDER_LOCATE&&CALL:OBJ_EXIST&&CALL:IMAGE_EXIST&&SET "LIVE_APPLY="&&SET "LOL_PARSE="&&SET "LIST_CREATE="
IF NOT DEFINED COLOR_SLOT SET "CLRTMP=%RANDOM%%RANDOM%"&&SET "CLRTMP=!CLRTMP:~5,5!"&&SET "CLRTMP=!CLRTMP:~1,1!"&&CALL COLOR %CLRTMP%
CLS&&TITLE  [%CD%\] $-h-@-Z-Z-@-m^^! A native command shell Windows image deployment tool.&&IF DEFINED COLOR_SLOT COLOR %COLOR_SLOT%
IF DEFINED PROG_OPER CALL:TITLECARD_NML
...
IF "%OBJ_IMAGEMGR%"=="VHDX" IF "%PROG_OPER%"=="PICK" IF EXIST "Z:\" SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH&&SET "PROG_OPER="&&GOTO:IMAGEMGR_START
IF "%OBJ_IMAGEMGR%"=="VHDX" IF "%PROG_OPER%"=="PICK" SET "VDISK_LETTER=Z"&&SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&CALL:VDISK_ATTACH
...
IF "%OBJ_IMAGEMGR%"=="WIM" SET "VDISK=%SCRATCHDIR%\SCRATCH.VHDX"&&SET "VHDX_SIZE=20000"
IF "%OBJ_IMAGEMGR%"=="WIM" CLS&&CALL:PAD_LINE&&ECHO                          Extracting Image....&&CALL:PAD_LINE&&CALL:TITLECARD&&SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE
IF "%OBJ_IMAGEMGR%"=="WIM" DISM /APPLY-IMAGE /IMAGEFILE:"%IMAGE_FOLDER%\%SELECT%" /INDEX:%INDEX_TEMP% /APPLYDIR:"Z:"
IF "%OBJ_IMAGEMGR%"=="VHDX" SET "INDEX_TEMP=1"&&SET "VDISK=%IMAGE_FOLDER%\%SELECT%"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_ATTACH
...
REG QUERY "%HIVE_SOFTWARE%\Microsoft\Windows\CurrentVersion\Component Based Servicing\Packages" /f Visibility /c /e /s>>"%TEMP%\COMPLST.TXT"
FOR /F "TOKENS=1-9 DELIMS= " %%a in (%TEMP%\COMPLST.TXT) DO (IF "%%b"=="Based" REG ADD "%%a %%b %%c" /V "Visibility" /T REG_DWORD /D "1" /F>NUL 2>&1)
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
...
IF DEFINED LIVE_APPLY FOR /F "TOKENS=1-9 DELIMS= " %%a in (%TEMP%\COMPLST.TXT) DO (IF "%%b"=="Based" REG ADD "%%a %%b %%c" /V "Visibility" /T REG_DWORD /D "2" /F>NUL 2>&1)
DEL %TEMP%\COMPLST.TXT >NUL
IF DEFINED LIVE_APPLY CALL:MOUNT_INT
IF NOT DEFINED LIVE_APPLY CALL:MOUNT_EXT
...
CALL:MOUNT_INT&&SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH&&CALL:SCRATCHDIR_DELETE
...
EXIT /B
:IMAGEMGR_RUN_LIST
SET "LIVE_APPLY="&&SET "LOL_PARSE="&&SET "LIST_CREATE="&&SET "IMAGE_TYPE_CREATE="&&SET "FILE_HEADER="&&SET "HEADER_TGT="&&SET "HEADER_TGT_LOL="&&CLS&&CALL:PAD_LINE&&ECHO                      $-h-@-Z-Z-@-m^^! File Picker&&CALL:PAD_LINE&&SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%CACHE_FOLDER%\*.LST"&&ECHO   AVAILABLE LST'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                     Enter File # To Select A List&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&CALL:MENU_SELECT
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" CALL:FILE_LIST >NUL
CALL:EXIT_FLAGGER
IF "%EXIT_FLAGGER%"=="1" EXIT /B
IF NOT EXIST "%CACHE_FOLDER%\%SELECT%" EXIT /B
...
IF NOT DEFINED LIVE_APPLY SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH
...
IF NOT DEFINED LIVE_APPLY IF "%OBJ_IMAGEMGR%"=="WIM" CALL:PAD_LINE&&ECHO                      Imaging operations complete&&CALL:PAD_LINE&&CALL:TITLECARD&&CALL:TEXTCOLOR
CALL:PAUSED
EXIT /B
:LOL_RUNNER
...
CALL:TEXTCOLOR
IF DEFINED LOL_PARSE COPY /Y "%HEADER_TGT_LOL%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
IF NOT DEFINED LOL_PARSE COPY /Y "%HEADER_TGT%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
IF "%FILE_HEADER%"=="IMAGE-APPLY" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL SET "IMAGEFILE_PACK=%PACK_FOLDER%\%%1"&&CALL:PACK_INSTALL)
IF "%FILE_HEADER%"=="RUN-ONCE" CALL:RUNONCE_CREATE&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for RunOnce Prep...&&CALL:PAD_LINE&&IF NOT "%%1"=="" IF EXIST "%PACK_FOLDER%\%%1" COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
IF "%FILE_HEADER%"=="SETUP-COMPLETE" CALL:SETUPCOMPLETE_CREATE&&FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Copying Package [%%1] for SetupComplete Prep...&&CALL:PAD_LINE&&IF NOT "%%1"=="" IF EXIST "%PACK_FOLDER%\%%1" COPY /Y "%PACK_FOLDER%\%%1" "%APPLYDIR_MASTER%\$">NUL)
...
IF "%FILE_HEADER%"=="FEATURES-ENABLE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (DISM /%APPLY_TARGET% /ENABLE-FEATURE /PACKAGENAME:"%%1" /ALL&&ECHO.)
IF "%FILE_HEADER%"=="FEATURES-DISABLE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Disabling Feature [%%1] ...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% /DISABLE-FEATURE /FEATURENAME:"%%1" /REMOVE&&ECHO.)
IF "%FILE_HEADER%"=="APPX-DELETE" FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%1 in (%TEMP%\%FILE_HEADER%.TXT) DO (CALL:PAD_LINE&&ECHO *Removing AppX Package [%%1] ...&&CALL:PAD_LINE&&DISM /%APPLY_TARGET% /REMOVE-Provisionedappxpackage /PACKAGENAME:"%%1"&&ECHO.)
...
IF NOT EXIST "%%a\*" CALL:TEXTCOLOR&&CALL:TITLECARD&&CALL:PAD_LINE&&ECHO  Installing...{NAME{%PackName%} {TYPE{%PackType%} {DESC{%PackDesc%}&&CALL:PAD_LINE
...
EXIT /B
:IMAGEMGR_DISM_MENU
SET "FILE_HEADER="&&SET "LIVE_APPLY="&&SET "DISM_OPER="
...
IF "%OBJ_IMAGEMGR%"=="WIM" CALL:SCRATCHDIR_CREATE
IF "%OBJ_IMAGEMGR%"=="VHDX" SET "VDISK=%TARGET_IMAGE%"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_ATTACH
IF "%OBJ_IMAGEMGR%"=="WIM" SET "VDISK=%SCRATCHDIR%\SCRATCH.VHDX"&&SET "VHDX_SIZE=20000"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE
...
CALL:PAD_LINE&&ECHO                       *End of DISM-Operations*&&CALL:PAD_LINE&&CALL:TITLECARD&&CALL:TEXTCOLOR&&SET "DISM_OPER="
EXIT /B
:IMAGEMGR_INSPECT
SET "LIVE_APPLY="&&SET "FILE_HEADER="&&SET "TARGET_IMAGE="
CLS&&CALL:PAD_LINE&&ECHO                      $-h-@-Z-Z-@-m^^! File Picker&&CALL:PAD_LINE
SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.%OBJ_IMAGEMGR%"&&SET "LIST_INSERTA= [ 0 ]\[Current-Environment]"&&ECHO   AVAILABLE %OBJ_IMAGEMGR%'S:&&CALL:FILE_LIST&&CALL:PAD_LINE&&ECHO                        Enter File # To Inspect&&CALL:PAD_LINE&&ECHO                Press (Enter) to Return to Previous Menu&&ECHO.&&CALL:MENU_SELECT&&CLS
...
IF "%OBJ_IMAGEMGR%"=="VHDX" SET "VDISK=%IMAGE_FOLDER%\%TARGET_IMAGE%"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_ATTACH
IF "%OBJ_IMAGEMGR%"=="WIM" SET "VDISK=%SCRATCHDIR%\SCRATCH.VHDX"&&SET "VHDX_SIZE=20000"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE
IF "%OBJ_IMAGEMGR%"=="WIM" CALL:PAD_LINE&&ECHO                           Extracting Image...&&CALL:PAD_LINE&&CALL:TITLECARD&&CALL:TEXTCOLOR
...
CALL:MOUNT_INT&&SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH
CALL:PAD_LINE>>"%PROG_SOURCE%\%TARGET_IMAGE%_REPORT.TXT"
...
CALL:PAD_LINE&&ECHO                       Full Inspection Complete&&CALL:PAD_LINE&&CALL:PAUSED
EXIT /B
...
SET "VDISK_LETTER=Z"&&CALL:VDISK_DETACH>NUL 2>&1
...
IF "%SOURCE_SLOT%"=="WIM" IF "%TARGET_SLOT%"=="WIM" SET "VDISK=%SCRATCHDIR_IMAGE%\SCRATCH.VHDX"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE
IF "%SOURCE_SLOT%"=="VHDX" IF "%TARGET_SLOT%"=="WIM" SET "VDISK_LETTER=Z"&&CALL:VDISK_ATTACH
IF "%SOURCE_SLOT%"=="WIM" IF "%TARGET_SLOT%"=="VHDX" SET "VHDX_SIZE=%VHDX_SIZE_IMAGE%"&&SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE
CALL:TITLECARD
...
CALL:PART_UNMOUNT
CALL:DISKMGR_ERASE
SET "DEPLOY_MODE="&&SET /A PART_CNT+=1
...
CALL:DISKMGR_ERASE
SET "DEPLOY_MODE=1"&&SET "BOOT_MSG=Drive incompatible, reverted to deploy-only drive."
...
(ECHO.select disk %DISK_NUMBER%&&ECHO.select partition 2&&ECHO.assign letter=S noerr&&ECHO.Exit)>"%TEMP%\DISK_PART"&&DISKPART /s "%TEMP%\DISK_PART"
IF EXIST "Q:\" IF EXIST "S:\" EXIT /B
IF "%PART_CNT%"=="4" ECHO The drive is currently in use or incompatible. Continue to try?&&ECHO  {Z}Continue {Enter}Abort&&CALL SET "PROMPT_SET=ARBIT123"&&CALL:PROMPT_SET
...
:VDISK_CREATE
REM VDISK_CREATE_VDISK_CREATE_VDISK_CREATE_VDISK_CREATE_VDISK_CREATE
IF NOT DEFINED VDISK_LETTER SET "VDISK_LETTER=Z"
...
:BOOT_MAKER
REM REPARTITION_REPARTITION_REPARTITION_REPARTITION_REPARTITION_REPARTITION
SET "PROG_OPER="&&SET "BOOT_MSG="&&SET "DISK_MSG="&&SET "PART_CNT="&&SET "PART_ERR="
IF "%CAME_FROM%"=="$ETUP" CLS
IF "%CAME_FROM%"=="$ETUP" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" IF NOT EXIST "%IMAGE_FOLDER%\%VHDX_SOURCE_$ETUP%" ECHO &&ECHO SOURCE VHDX NOT SET&&PAUSE&&EXIT /B
...
IF "%PROG_MODE%"=="RAMDISK" IF NOT EXIST "Q:\$.WIM" ECHO &&ECHO NO BOOT MEDIA DETECTED&&CALL:EFI_UNMOUNT&&PAUSE&&EXIT /B
IF EXIST "%TEMP%\$.WIM" DEL /Q /F "\\?\%TEMP%\$.WIM">NUL 2>&1
IF EXIST "%TEMP%\BOOT-MEDIA.SAV" DEL /Q /F "\\?\%TEMP%\BOOT-MEDIA.SAV">NUL 2>&1
IF "%PROG_MODE%"=="RAMDISK" SET "BOOT_IMAGE=Q:\$.WIM"
COPY /Y "%BOOT_IMAGE%" "%TEMP%\$.WIM">NUL 2>&1
...
IF DEFINED BOOT_ABT SET "BOOT_MSG=%BOOT_MSG0%"&&GOTO:BOOT_CLEANUP
:BOOT_CREATE
DISM /cleanup-MountPoints>NUL 2>&1
...
MOVE /Y "\\?\%TEMP%\$.WIM" "%SCRATCHDIR_BOOT%">NUL 2>&1
SET "VDISK=%SCRATCHDIR_BOOT%\SCRATCH.VHDX"
SET "VDISK_LETTER=Z"&&CALL:VDISK_CREATE>NUL 2>&1
...
IF NOT EXIST "Q:\EFI\Boot\bootx64.efi" SET "BOOT_MSG=EFI missing"&&GOTO:BOOT_CLEANUP
CALL:BCD_CREATE>NUL 2>&1
IF NOT EXIST "Q:\EFI\Microsoft\Boot\BCD" SET "BOOT_MSG=BCD missing"&&GOTO:BOOT_CLEANUP
ECHO SAVING BOOT-MEDIA...&&CALL:TITLECARD_NML
ECHO "%%SYSTEMDRIVE%%\$\$HAZZAM.CMD">"%APPLYDIR_BOOT%\WINDOWS\SYSTEM32\STARTNET.CMD"
DISM /CAPTURE-IMAGE /IMAGEFILE:"Q:\$.WIM" /CAPTUREDIR:"%CAPTUREDIR_BOOT%" /NAME:NAME /CheckIntegrity /Verify /Bootable
...
IF "%PROG_MODE%"=="COMMAND" ECHO COPYING %VHDX_SOURCE_$ETUP%...&&COPY /Y "\\?\%IMAGE_FOLDER%\%VHDX_SOURCE_$ETUP%" "%LAST_LETTER%:\$">NUL 2>&1
IF "%CAME_FROM%"=="$ETUP" IF "%BCD_SYSTEM%"=="NAME" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" ECHO COPYING %VHDX_SOURCE_$ETUP%...&&COPY /Y "\\?\%IMAGE_FOLDER%\%VHDX_SOURCE_$ETUP%" "%LAST_LETTER%:\$">NUL 2>&1
IF "%CAME_FROM%"=="$ETUP" IF "%BCD_SYSTEM%"=="SLOT" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" ECHO COPYING %VHDX_SOURCE_$ETUP%...&&COPY /Y "\\?\%IMAGE_FOLDER%\%VHDX_SOURCE_$ETUP%" "%LAST_LETTER%:\$\%VHDX_SLOT_$ETUP%.VHDX">NUL 2>&1
IF "%CAME_FROM%"=="$ETUP" IF NOT "%VHDX_SOURCE_$ETUP%"=="-SELECT-" IF NOT EXIST "%LAST_LETTER%:\$\*.VHDX" SET "BOOT_MSG=ERROR COPYING %VHDX_SLOT_$ETUP%.VHDX"
:BOOT_ABT
