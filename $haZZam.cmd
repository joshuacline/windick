...
EXIT /B
:LOL_RUNNER
FOR /F "TOKENS=1 SKIP=1 DELIMS={}" %%a in (%HEADER_TGT%) DO (IF EXIST "%CACHE_FOLDER%\%%a" CALL SET "HEADER_TGT=%CACHE_FOLDER%\%%a"&&CALL:HEADER_PARSE_TMP)
EXIT /B
:HEADER_PARSE_TMP
SET "RUNFLAG="&&SET /P FILE_HEADER=<"%HEADER_TGT%"
...
CALL:TEXTCOLOR&&COPY /Y "%HEADER_TGT%" "%TEMP%\%FILE_HEADER%.TXT">NUL 2>&1
