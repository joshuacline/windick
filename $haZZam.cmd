...
IF "%PROG_OPER%"=="MOVE2HOME" SET "LIST_FMT=NUMERAL"&&SET "LIST_DIR=%IMAGE_FOLDER%\*.VHDX"&&CALL:FILE_LIST&&CALL:PAD_LINE
...
IF NOT DEFINED PROG_OPER IF NOT "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="ISOLATED" ECHO                        {-}BOOT[FOLDER]IMAGE{+}&&CALL:PAD_LINE
IF NOT DEFINED PROG_OPER IF "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="UNIFIED" ECHO                     ~ {R}ebuild as [%BCD_SYSTEM%-MODE] } ~&&CALL:PAD_LINE
IF NOT DEFINED PROG_OPER IF "%PROG_MODE%"=="RAMDISK" IF "%FOLDER_MODE%"=="ISOLATED" ECHO     {-}BOOT[FOLDER]IMAGE{+}          ~ {R}ebuild as [%BCD_SYSTEM%-MODE] } ~&&CALL:PAD_LINE
IF DEFINED PROG_OPER ECHO                  Enter File # To [%PROG_OPER%] {/}Type&&CALL:PAD_LINE
ECHO                Press (Enter) to Return to Previous Menu
CALL:MENU_SELECT&&IF DEFINED PROG_OPER CALL:FILE_LIST>NUL
...
IF "%SELECT%"=="-" IF "%FOLDER_MODE%"=="ISOLATED" SET "PROG_OPER=MOVE2HOME"&&GOTO:$ETUP_START
...
IF "%PROG_OPER%"=="MOVE2HOME" IF EXIST "%PROG_SOURCE%\%SELECT%" GOTO:$ETUP_START
...
IF "%PROG_OPER%"=="MOVE2HOME" IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%" GOTO:$ETUP_START
IF "%PROG_OPER%"=="MOVE2HOME" IF NOT EXIST "%IMAGE_FOLDER%\%SELECT%\*" CALL:PAD_LINE&&ECHO  Moving [%IMAGE_FOLDER%\%SELECT%]to[%PROG_SOURCE%]...&&CALL:PAD_LINE&&MOVE /Y "%IMAGE_FOLDER%\%SELECT%" "%PROG_SOURCE%\"
